        ...
        <table border="1">
            ...
            <c:forEach var="emp" items="${employeeList}" varStatus="status">
                <tr>
                    ...
                    <td>
                        <c:choose>
                            <c:when test="${emp.status == 'Đang làm'}"><span
                                    style="color:green">${emp.status}</span></c:when>
                            <c:when test="${emp.status == 'Nghỉ phép'}"><span
                                    style="color:orange">${emp.status}</span></c:when>
